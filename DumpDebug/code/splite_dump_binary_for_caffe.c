...
static char *file_out_name = "output";
BOOL bFloat = TRUE;
...
BOOL isHeadLine0(char *buf, char *pfile_name, int *output_idx, int *dims, int *shape)
{
    char *p = NULL;
    char in_out_str[8];
    int file_num = 0;
    ...
    //find shape if dims is 4
    if (*dims == 4)
    {
        p = strstr(buf,"shape:");
        if (p == NULL)
            return FALSE;
        sscanf(p, "shape:[%d, %d, %d, %d]", &shape[0],&shape[1],&shape[2],&shape[3]);
    }
...
BOOL spliteFile(FILE *file, const char *pfileName)
{
    ...
    do {
        ...
        if (buffer_size == 0)
        {
            ...
            if (isHeadLine0(buffer0, file_name, &output_idx, &dims, &shape) == TRUE)
            {
        ...
        else
        {
            ...
            if (fData != NULL)
            {
                        ...
                        for(int n = 0;n < shape[0];n++)
                        {
                           for(int c = 0;c < shape[3];c++)
                           {
                              for(int h = 0;h < shape[1];h++)
                              {
                                ...
                                {
                                    offset = n*shape[1]*shape[2]*shape[3]+ h*shape[2]*shape[3] + w*shape[3] + c;
                                    sprintf(buf, "%.6f\n",*(f_Data+offset));
                                    fputs(buf, fData);
                ...
                {
                    perror("don't support fixed model's bin file");
