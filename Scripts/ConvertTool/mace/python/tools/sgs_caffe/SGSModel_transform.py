...
class TransformToSchema(object):
    ...
    def check_NCHW_And_Change(self, input_name, op_name):
        # if special operators such as pool and conv 's input dataformat is 'NHWC', this function will be executed
        # When the input flag is NHWC,
        # the input shape will be displayed as the shape in the original model, and will not be displayed as the shape after [0,2,3,1] dimension exchange
        # so it is necessary to insert a transpose operator. force exchange shape
    ...
    def split_Crop(self, op):
        ...
        if len(output_shape) == 4:
          ...
          begin_tensor_data = [bn,bh,bw,bc] #nchw -> nhwc
        else:
        ...
        if len(output_shape) == 4:
          ...
          size_tensor_data = [n,h,w,c] #nchw -> nhwc
        else:
    ...
    def split_Clip(self, op):
        for op_ in self._SGSModel.op:
            if op_ == op:
                ...
                if minTensor_data == 0 and maxTensor_data == 6:
                    op_.type = "RELU6"
                    self._maceOpArray = np.append(self._maceOpArray,op_)
                else:
    ...
    def split_Deconv2D(self, op):
        ...
        op_dilation.name = 'Dilation'+ op.name
    ...
    def split_Eltwise(self, op):
        for op_ in self._SGSModel.op:
            if op_ == op:
                ...
                if len(op_.input) > 1 :
                    # convert const tensors' shape like [n, 1, 1] to [n]
                    all_NCHW_tensor_is_scalar = True
                    ...
                    for inputName in op_.input:
                        inputTensorN = self.find_tensor_by_name(inputName)
                        if inputTensor1.data_format != inputTensorN.data_format:
                            all_data_format_same = False
                        if inputTensorN.data_format == mace_pb2.DT_NCHW and self.is_scalar_tensor(inputName) == False:
                            all_NCHW_tensor_is_scalar = False

                    #do check_NCHW_And_Change in following cases:
                    #input1: DT_NHWC input2: DT_NCHW => check_NCHW_And_Change(input1)
                    #input1: DT_NHWC input2: vector  => check_NCHW_And_Change(input1)
                    #input1: DT_NHWC input2: scalar  => check_NCHW_And_Change(input1)
                    #input1 input2 could be inversed
                    if (all_data_format_same == False):
                        for i in six.moves.range(len(op_.input)):
                            inputName = op_.input[i]
                            inputTensorN = self.find_tensor_by_name(inputName)

                            if (len(inputTensorN.dims)==4 and inputTensorN.data_format == mace_pb2.DT_NHWC and all_NCHW_tensor_is_scalar == False):
                                #add for onnx_litehrnet_18_mpii_256x256
                                change_index = i
                            ...
                            #six.print_("Eltwise layer shape not support name = {} ,data_format {} vs {},shape {} vs {}".format
                            #(op_.name,inputTensor1.data_format,inputTensor2.data_format,inputTensor1.dims,inputTensor2.dims))
                            #assert 0

                self._maceOpArray = np.append(self._maceOpArray, op_)
                if change_format_enable:
                    is_input_NHWC_exist = False
                    is_input_all_NHWC = True
                    for inputName in op_.input:
                        inputTensorN = self.find_tensor_by_name(inputName)
                        if inputTensorN.data_format == mace_pb2.DT_NHWC:
                            is_input_NHWC_exist = True
                        if inputTensorN.data_format == mace_pb2.DT_NCHW:
                            is_input_all_NHWC = False

                    if len(op_.input) > 1 and is_input_NHWC_exist:
                        if is_input_all_NHWC:
                            for outputName in op_.output:
                                outputTensor = self.find_tensor_by_name(outputName)
                                outputTensor.data_format = mace_pb2.DT_NHWC
                        else:
                            for inputName in op_.input:
                                inputTensorN = self.find_tensor_by_name(inputName)
                                if inputTensorN.data_format == mace_pb2.DT_NHWC and len(inputTensorN.dims)==4:
                                    for outputName in op_.output:
                                        outputTensor = self.find_tensor_by_name(outputName)
                                        outputTensor.data_format = mace_pb2.DT_NHWC
    ...
    def split_FullyConnected(self, op):
        ...
        if len(input_shape) == 4 and (input_shape[1] != weight_shape[1]):
           #case for lenet
           [n,c,h,w] = input_shape
           # add reshape
           op_reshape = self._SGSModel.op.add()
           ...
           self.remove_op_with_name(op_name)
             ...
             for op_ in self._SGSModel.op:
               if op_ == op:
                 ...
                 mace_check(innershape == weight_shape[1],
    ...
    def split_LSTM(self, op):
        ...
        op_SGS_LSTM.name = 'SGS_LSTM' + op.name
    ...
    def split_Pooling(self, op):
        ...
        for op_ in self._SGSModel.op:
            if op_ == op:
              ...
              if inputTensor.data_format == mace_pb2.DT_NHWC:
                # When the input flag is NHWC,
                # the input shape in debug.sim will be displayed as the shape in the original model,
                # and will not be displayed as the shape after [0,2,3,1] dimension exchange,
                # so it is necessary to insert a transpose operator. force exchange shape
                new_input_name = self.check_NCHW_And_Change(xi, op_.name)
                op_.input[0] = new_input_name
                # The output flag is infected by the input (mace_pb2.DT_NHWC)
                # but due to the characteristics of the operator,
                # if the shape of the input is changed by inserting transpose,
                # the dimension of the output should also be exchanged, which can be achieved by changing the flag of outputtensor
                self.set_output_datatype_by_input (op_, mace_pb2.DT_NCHW)
              arg = op_.arg
              for i in six.moves.range(len(arg)):
                name = arg[i].name
    ...
    def split_Slice(self, op):
        ...
        if use_slice:
            for i in six.moves.range(len(arg)):
                name = arg[i].name
                if name == MaceKeyword.mace_axis_str:
                    arg[i].i = self.get_axes_by_shape (arg[i].i, len(inputShape),0)
                    axis = arg[i].i
                if name == "slice_point":
            ...
            for i in six.moves.range(len(op.output_shape)):
                ...
                if len(inputShape) == 4 and inputTensor.data_format == mace_pb2.DT_NCHW:
                    axis_begin = self.get_axes_by_shape(axis, len(inputShape))
                    ...
                    if op_ == op:
                        ...
                        for i in six.moves.range(len(arg)):
                            ...
                            if name == MaceKeyword.mace_axis_str:
                                axis_tensor_name = op_.name + '_axis'
                                axis = op_.arg[i].i
                                axis = self.get_axes_by_shape(axis, len(inputShape), 0)
                ...
                for op_ in self._SGSModel.op:
                    if op_ == op:
                        ...
                        for i in six.moves.range(len(arg)):
                            name = arg[i].name
                            if name == MaceKeyword.mace_axis_str:
                                axis = self.get_axes_by_shape(arg[i].i, len(inputShape), 0)
                                arg[i].i = self.get_axes_by_shape(arg[i].i, len(inputShape), 1)
                                axis_tensor_name = op_.name + '_axis'
        ...
        for op_ in self._SGSModel.op:
            if op_ == op:
                if len(op_.input) > 1 :
                    ...
                    # check dataformat of both input
                    new_x1 = op_.input[0]
                    new_x2 = op_.input[1]
                    inputTensor1 = self.find_tensor_by_name(new_x1)
                    input_shape1 = self.get_shape_by_name(new_x1)
                    inputTensor2 = self.find_tensor_by_name(new_x2)
                    input_shape2 = self.get_shape_by_name(new_x2)
                    if self.is_const_tensor(new_x2) == False:
                        if inputTensor1.data_format != inputTensor2.data_format:
                            all_data_format_same = False
                            for i in six.moves.range(len(op_.input)):
                                inputName = op_.input[i]
                                inputTensorN = self.find_tensor_by_name(inputName)

                                if len(inputTensorN.dims)==4 and inputTensorN.data_format == mace_pb2.DT_NHWC:
                                    #add for onnx_litehrnet_18_mpii_256x256
                                    change_index = i
                                    new_input_name = self.check_NCHW_And_Change(op_.input[change_index], op_name)
                                    op_.input[change_index] = new_input_name
                    ...
                    if len(op_.input) == 3: #has bias
                        ...
                        op_mul.output_shape.extend(op_.output_shape)
                        self._maceOpArray = np.append(self._maceOpArray,op_mul)
                        op_add = self._SGSModel.op.add()
                        op_add.name = name + '_add'
                        op_add.type = 'ADD'
                        ...
                        op_add.output_shape.extend(op_.output_shape)
                        self._maceOpArray = np.append(self._maceOpArray,op_add)
                    else:
                        op_mul = self._SGSModel.op.add()
                        op_mul.name = op_name + '_mul'
    ...
    def split_Transpose(self, op):
        ...
        if inputTensor.data_format == mace_pb2.DT_NCHW and intputShapeSize == 4:
          ...
          self.set_output_datatype_by_input (op, mace_pb2.DT_NHWC)
    ...
    def split_Reorg(self, op):
        ...
        op_reshape.input.extend([output_op_transpose])
        reshape_output_tensor_name = op_reshape.name + '_output_shape'
        reshape_output_tensor_data = [n,int(4*c),h,w]
        reshape_output_tensor_shape = [len(reshape_output_tensor_data)]
        self.add_tensor(self._SGSModel, reshape_output_tensor_name, reshape_output_tensor_shape,
                        mace_pb2.DT_INT32, reshape_output_tensor_data)
        op_reshape.input.extend([reshape_output_tensor_name])
    ...
    def split_Reverse(self, op):
        ...
        for i in six.moves.range(len(arg)):
          name = arg[i].name
          if name == MaceKeyword.mace_axis_str:
            arg[i].i = self.get_axes_by_shape (arg[i].i, len(input_shape))
            axis = arg[i].i
      ...
      #   Rearranged as NHWC
        for op_ in self._SGSModel.op:
            if op_ == op:
                op_name = op_.name
                xi = op_.input[0]
                [n,c,h,w] = inputShape = self.get_shape_by_name(xi)
                arg = op_.arg
                for i in six.moves.range(len(arg)):
                    name = arg[i].name
                    if name == "group":
                ...
                op_transpose.output_shape.add()
                op_transpose_shape = copy.deepcopy(inputShape)
                ...
                op_reshape.output_shape[0].dims.extend([n,c,h,w])
                self.add_tensor(self._SGSModel, output_op_reshape, output_shape,
                                mace_pb2.DT_FLOAT, None, data_fromat = mace_pb2.DT_NHWC)
                self._maceOpArray = np.append(self._maceOpArray,op_reshape)

                #5\creat transpose
                op_transpose = self._SGSModel.op.add()
                op_transpose.name = op_name + '_transpose3'
                op_transpose.type = 'TRANSPOSE'
                shape_tensor_name = op_transpose.name + '_shape'
                shape_tensor_data = [0,2,3,1]
                shape_tensor_shape = [4]
