...
class Net(calibrator_custom.SIM_Simulator):
    def __init__(self, model_path, batch_size, log, input_config):
        super().__init__()
        self.model = calibrator_custom.offline_simulator(model_path, batch=batch_size, show_log=log, input_config=input_config)
...
def arg_parse():
    parser = argparse.ArgumentParser(description='Multi Batch Simulator Tool')
...
def main():
    ...
    net = Net(model_path, batch, log, input_config)
    printf(str(net))
    result = net(img_gen)
