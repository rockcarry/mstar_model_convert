...
def getBNOutShape(input_shape):
    ...
    return output_shape
#构建节点
def createBN(layer, nodename, inname, outname, input_shape, boom=False):
    ...
    #构建node
    if boom:
      node = Node.c2oNode(layer, nodename, "BatchNormalization", inname, outname, input_shape, output_shape, dict, True)
    else:
