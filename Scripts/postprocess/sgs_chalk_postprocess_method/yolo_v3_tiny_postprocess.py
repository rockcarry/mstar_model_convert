...
def buildGraph(model_config):
    ...
    sx = list(np.ones(side_x[0]*side_y[0]*box_num)*(1.0/side_y[0]))+list(np.ones(side_x[1]*side_y[1]*box_num*(1.0/side_y[1])))
    sy = list(np.ones(side_x[0]*side_y[0]*box_num)*(1.0/side_x[0]))+list(np.ones(side_x[1]*side_y[1]*box_num*(1.0/side_x[1])))
    #sy = sx

    biases0= [[81,82],[135,169],[344,319]]
    biases1= [[10,14],[23,27],[37,58]]
    sw =[x[0]/(2*input_width) for x in biases0 ]*(side_x[0]*side_y[0])  + [x[0]/(2*input_width) for x in biases1 ]*(side_x[1]*side_y[1])
    sh =[x[1]/(2*input_height) for x in biases0 ]*(side_x[0]*side_y[0])  + [x[1]/(2*input_height) for x in biases1 ]*(side_x[1]*side_y[1])
    ...
    concat_tensor = sgs_chalk.Concatenation([reshape_tensor_0,reshape_tensor_1],axis=1,name="concat_tensor")
    ...
    out1 = sgs_chalk.TFLite_Detection_NMS(bosdecoder_output_list[0],bosdecoder_output_list[1],bosdecoder_output_list[2],
                                          ...
                                          mode='YOLO',max_detections=100,nms_score_threshold=0.00499999989,
...
def get_postprocess():
    model_config = {"name":"yolo_v3_tiny",
          "input" :  ["detector/yolo-v3-tiny/Conv_9/BiasAdd", "detector/yolo-v3-tiny/Conv_12/BiasAdd"],
