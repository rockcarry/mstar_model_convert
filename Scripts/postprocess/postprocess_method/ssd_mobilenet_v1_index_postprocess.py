...
def BoxDecoder(sgs_builder,model_config,unpack_output_tensors):
    if 'TOP_DIR' in os.environ:
        ...
        anchors = np.load(os.path.join(Project_path, "SRC/Tool/Scripts/postprocess/", "anchors_ssd_tf.npy"))
...
def buildGraph(sgs_builder,model_config):
    ...
    cus_options = [(b"scores_lengh",90,"int"),
                   (b"skip",1,"int")]
...
def get_postprocess():
    ...
    outfilename = model_config["name"] + "_index_postprocess.sim"
