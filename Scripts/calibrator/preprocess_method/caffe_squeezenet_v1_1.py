...
def get_image(img_path, resizeH=227, resizeW=227, resizeC=3, norm=True, meanB=104, meanG=117, meanR=124, std=1, rgb=False, nchw=False):
    ...
    if h < w:
        ...
        off = (h - w) // 2
        im = im[off:off + h, :]
    im = resize_image(im, [resizeW, resizeH])
    im = im * 255
    if norm:
        _mean = np.array([meanB, meanG, meanR]).reshape([1,1,3])
        im = im - _mean
        _im = im.astype(np.float32)
    else:
        _im = np.uint8(im)
