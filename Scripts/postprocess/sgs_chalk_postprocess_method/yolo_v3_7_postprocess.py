...
def buildGraph(model_config):
    ...
    reshape0_tensor = sgs_chalk.Reshape(in0,[1,side_x[0]*side_y[0]*box_num,num_classes+5],name="reshape0_tensor")
    in1 = sgs_chalk.Input(model_config["input_shape"][1], name = model_config["input"][1])
    reshape1_tensor = sgs_chalk.Reshape(in1,[1,side_x[1]*side_y[1]*box_num,num_classes+5],name="reshape1_tensor")
    in2 = sgs_chalk.Input(model_config["input_shape"][2], name = model_config["input"][2])
    reshape2_tensor = sgs_chalk.Reshape(in2,[1,side_x[2]*side_y[2]*box_num,num_classes+5],name="reshape2_tensor")
    concat_tensor = sgs_chalk.Concatenation([reshape0_tensor,reshape1_tensor,reshape2_tensor],axis=1,name="concat_tensor")
    unpack_out_tensors1 = []
    for i in range(7):
