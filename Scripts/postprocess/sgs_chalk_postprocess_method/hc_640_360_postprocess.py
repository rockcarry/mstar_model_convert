...
if 'IPU_TOOL' in os.environ:
    ...
    prior_bbox = np.load(os.path.join(Project_path, "Scripts/postprocess/", "anchors_hc_640_360.npy"))
...
def buildGraph(model_config):
    ...
    postprocess_max_output_list = sgs_chalk.PostProcess_Max(convert_scores,num_classes=21)
    ...
    out1 = sgs_chalk.TFLite_Detection_NMS(bosdecoder_output_list[0],bosdecoder_output_list[1],bosdecoder_output_list[2],
                                          ...
                                          nms_iou_threshold=0.45,num_classes=1,is_need_index=False)
...
def get_postprocess():
    model_config = {"name":"hc_640_360",
          "input" : ["402","403"],
          "input_shape" : [[1,3723,4],[1,3723,2]],
          "shape" : [1,3723],
