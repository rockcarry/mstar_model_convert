...
from mace.python.tools.caffe import SGSModel_transform
...
class ConvertSGSModel(object):
    ...
    def __init__(self, net, inputName, inputShapeMap, outputName, outputDir, input_pack_model, output_pack_model):
        self._net = net
        self._netInputName = inputName
        self._netInputShapeMap = inputShapeMap
    ...
    def convert(self):
        transform = SGSModel_transform.TransformToSchema(self._net,self._netInputName,self._netInputShapeMap,self._netOutputName, self._input_pack_model, self._output_pack_model)
    ...
    def _creatBuffer(self):
        ...
        dutime1 = (end1 - start1).seconds
        print("creat buffer time = '{}s'".format(dutime1))
        #####################
    ...
    def _creatTensor(self):
        utility.annPrint("=== creat Tensor begin ===")
        ...
        for tensor in self._tensorArray:
          ...
          index -= 1
        utility.annPrint("=== creat Tensor end ===")
        return _SGSTensorArray

    def _creatOperator(self,SGSMode):
        utility.annPrint("=== creat Operator begin ===")
    ...
    def _creatModel(self, SGSOperatorArray, SGSSubGraphArray, SGSBufferArray):
        ...
        utility.annPrint("=== creat Model end ===")
        return buf
    ...
    def getDataFromFlexBuffer(self,op):
        ...
        if op.name == 'TFLite_CaffeSSD_Detection_PostProcess':
            #set data
        ...
        if op.name == 'TFLite_RFCSSD_Detection_PostProcess':
            ...
            lib.insertFloatString.restype = None
            lib.insertFloatString(b'w_scale\0', 5.0)
        elif op.name == 'TFLite_Detection_NMS':
            pass
        elif op.name == 'TFLite_YoloV2_Detection_PostProcess':
            lib.insertFloatString.argtypes = [c_char_p,c_float]
            lib.insertFloatString.restype = None
        ...
        elif op.name == 'TFLite_YoloV2_608_Detection_PostProcess':
            lib.insertIntString.argtypes = [c_char_p,c_int]
            lib.insertIntString.restype = None
            lib.insertIntString(b'max_detections\0',100)
            lib.insertIntString.argtypes = [c_char_p,c_int]
        ...
        elif op.name == 'TFLite_YoloV3_Detection_PostProcess':
            lib.insertIntString.argtypes = [c_char_p,c_int]
            lib.insertIntString.restype = None
            lib.insertIntString(b'max_detections\0',100)
            lib.insertIntString.argtypes = [c_char_p,c_int]
        ...
        elif op.name == 'TFLite_LanceNet_Detection_PostProcess':
            lib.insertIntString.argtypes = [c_char_p,c_int]
            lib.insertIntString.restype = None
            lib.insertIntString(b'max_detections\0',100)
            lib.insertIntString.argtypes = [c_char_p,c_int]
        ...
        elif op.name == 'TFLite_FDA_Detection_PostProcess':
            lib.insertIntString.argtypes = [c_char_p,c_int]
            lib.insertIntString.restype = None
            lib.insertIntString(b'max_detections\0',100)
            lib.insertIntString.argtypes = [c_char_p,c_int]
    ...
    def builtin_ADD_N(self, op):
        ...
        #creat Operator Code
        tflite.OperatorCode.OperatorCodeStart(self._builder)
        tflite.OperatorCode.OperatorCodeAddBuiltinCode(self._builder, tflite.BuiltinOperator.BuiltinOperator().ADD_N)
        operatorCode = tflite.OperatorCode.OperatorCodeEnd(self._builder)
    ...
    def builtin_CUSTOM(self, op):
        ...
        elif name.lower().find('roi') == 0:
            #op.name = "RoiPooling"
            customCode = self._builder.CreateString("RoiPooling")
        else:
