...
class c2oNode(object):
    def __init__(self,layer,node_name,type,inputs_name,outputs_name,inputs_shape,outputs_shape,dict={}, Flag=False, value = None):
        if type == 'Constant':
          tensor_shape = tuple(outputs_shape[0])
          self.node = helper.make_node(
              'Constant',
              inputs=[],
              ...
              value=helper.make_tensor(
                  ...
                  dims=tensor_shape,
                  vals=value.flatten().astype(float),
              ),
           )
        else:
